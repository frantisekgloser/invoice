...
RSpec.describe "outgoing_invoice_items/edit.html.erb", type: :view do
  ...
  before do
    ...
    @outgoing_invoice_item = OutgoingInvoiceItem.create!(amount: '123', note: 'test_outgoing_invoice_item_note', outgoing_invoice_id: @outgoing_invoice.id, item_id: @item.id, user_id: @user.id)
  end

  it "edits Outgoing invoice item with id: as parameter" do

    controller.extra_params = { :outgoing_invoice_id=>@outgoing_invoice.id, :id=>@outgoing_invoice_item.id }
    expect(controller.request.fullpath).to eq edit_outgoing_invoice_outgoing_invoice_item_path(@outgoing_invoice, @outgoing_invoice_item)
  end  
end
